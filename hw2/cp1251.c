...
static int cp1241_unicode[] = {
    ...
    0x0452, 0x2018, 0x2019, 0x201C, 0x201D, 0x2022, 0x2013, 0x2014, 0xffff,
...
bool cp1251_to_unicode(uint8_t* in, uint32_t* out) {
  uint32_t index = cp1251[*in];
  if (index >= sizeof(cp1241_unicode)) {
    return false;
  }

  *out = cp1241_unicode[index];
  return true;
}
